...
if __name__ == "__main__":
    person_x_image_path = "referencia/Screenshot_1.png"
    ...
    if person_x_features is not None:
        ...
        for idx, (image_file, match) in enumerate(zip(os.listdir(images_y_folder_path), matches_list)):
            print(f"Imagem {idx+1}: {image_file} - Correspondências: {match}")
            
            # If there's a match, copy the image to the 'correspondencias' folder
            if match != "Desconhecido":
                image_src_path = os.path.join(images_y_folder_path, image_file)
                image_dst_path = os.path.join(correspondence_folder_path, image_file)
